...
async def main(base_model: str = BASE_MODEL, epochs: int = 50, num_rollouts: int = 50, hf_repo_id: str = None, push_to_hub: bool = True, run_demo: bool = False):
    """
    ...
    if run_demo:
        try:
            ...
            for _ in tqdm.tqdm(range(NUM_GAMES)):
                try:
                    result = play_demo_game(lora_model_path)
                    if result:
                        results.append({"messages": result})
                except:
                    continue

            jsonl_path = "winning_games.jsonl"
            with open(jsonl_path, "w") as f:
                for item in results:
                    f.write(json.dumps(item) + "\n")

            dataset_repo_id = f"banachspace/tic-tac-toe-wins-{project_title}"
            dataset_info = push_dataset_to_hub(jsonl_path, dataset_repo_id)
            print(f"🚀 Dataset uploaded to: {dataset_info['repo_url']}")
            print()
            print(f"\tDemo games completed: {100*len(results)/NUM_GAMES:.2f}% successful.")
            print()
...
def push_dataset_to_hub(jsonl_path, repo_id):
    """
    Push a dataset (JSONL format) to the Hugging Face Hub.

    Args:
        jsonl_path: Path to the dataset file (.jsonl)
        repo_id: Hugging Face dataset repo ID (e.g. 'banachspace/tic-tac-toe-wins')
